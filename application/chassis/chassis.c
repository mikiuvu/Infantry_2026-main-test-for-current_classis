...
#define SLIP_DEVIATION_RATIO     0.3f    // 偏差比例阈值 (30%)
...
/* DEBUG: 用于验证编译的是哪个项目 */
static uint32_t THIS_IS_CURRENT_CLASSIS_PROJECT = 0x12345678;
static uint8_t chassis_cmd_struct_size = sizeof(Chassis_Ctrl_Cmd_s);
...
void ChassisInit()
{
    ...
    PID_Init_Config_s observer_pid_conf = {
        ...
        .Ki = 2.0f,         // 消除稳态误差
        .Kd = 0.5f,
        .MaxOut = 50.0f,    // 最大补偿力 (N) - 不能太大，主要靠速度环
        .IntegralLimit = 20.0f,
    ...
    CANComm_Init_Config_s comm_conf = {
        ...
        // TODO: 临时修复！云台发送65字节，需要重新编译云台代码使两端一致
        .recv_data_len = 65,  // 原本是 sizeof(Chassis_Ctrl_Cmd_s)
...
static void GlobalObserverCalculate()
{
    ...
    // 应用TCS因子限制前馈
    float tcs = slip_ctrl.tcs_factor;
    
    current_ff_lf = F_lf * force_to_torque * FF_CURRENT_SCALE * tcs;
    current_ff_rf = F_rf * force_to_torque * FF_CURRENT_SCALE * tcs;
    current_ff_lb = F_lb * force_to_torque * FF_CURRENT_SCALE * tcs;
    current_ff_rb = F_rb * force_to_torque * FF_CURRENT_SCALE * tcs;
...
{   
    ...
    // chassis_power_buff = chassis_cmd_recv.chassis_power_buff * 0.75;
    // switch (referee_data->GameRobotState.chassis_power_limit)
    // {

    // case 60:
    //     rotate_speed_buff = 1.4;
    //     chassis_power_buff = 1.6;
    //     break;
    // case 65:
    //     rotate_speed_buff = 1.5;
    //     chassis_power_buff = 1.7;
    //     break;    
    // case 70:
    //     rotate_speed_buff = 1.6;
    //     chassis_power_buff = 1.85;
    //     break;    
    // case 75:
    //     rotate_speed_buff = 1.70;
    //     chassis_power_buff = 1.9;
    //     break;    
    // case 80:
    //     rotate_speed_buff = 1.75;
    //     chassis_power_buff = 2.0;
    //     break;
    // case 85:
    //     rotate_speed_buff = 1.80;
    //     chassis_power_buff = 2.1;
    //     break;
    // case 90:
    //     rotate_speed_buff = 1.85;
    //     chassis_power_buff = 2.2;
    //     break;    
    // case 95:
    //     rotate_speed_buff = 1.90;
    //     chassis_power_buff = 2.3;
    //     break;
    // case 100:
    //     rotate_speed_buff = 1.95;
    //     chassis_power_buff = 2.4;
    //     break;
    // case 120:
    //     rotate_speed_buff = 2;
    //     chassis_power_buff = 1.6;
    ...
    //     chassis_power_buff = 0.75;
    //     break;
    // }

    // switch (chassis_cmd_recv.dash_mode)
    // {
    // case DASH_OFF:
    //     ChassisPowerSet((float)referee_data->GameRobotState.chassis_power_limit - 5);
    //     break;
    
    // case DASH_ON:
    //     ChassisPowerSet((float)referee_data->GameRobotState.chassis_power_limit + 100);
    //     break;
    // default:
    //     break;
    // }
...
static void EstimateSpeed()
{
    ...
    float imu_accel_x = Chassis_IMU_data->MotionAccel_b[0] * 1000.0f;
    float imu_accel_y = Chassis_IMU_data->MotionAccel_b[1] * 1000.0f;
    
    // IMU安装偏移补偿: a_center = a_imu - omega × (omega × r) - alpha × r
    // 简化处理: 只补偿角速度引起的向心加速度项
    // 向心加速度: a_cent = -omega^2 * r (指向旋转中心)
    float omega = Chassis_IMU_data->Gyro[2];  // rad/s (Z轴角速度)
    float omega_sq = omega * omega;
    
    // 补偿向心加速度 (IMU测量值减去向心加速度得到底盘中心加速度)
    imu_accel_x -= omega_sq * CHASSIS_IMU_OFFSET_X;  // 向心加速度指向中心
    imu_accel_y -= omega_sq * CHASSIS_IMU_OFFSET_Y;
    ...
    VOFA(chassis_feedback_data.real_vy,chassis_feedback_data.real_vx);
    // 记录轮速用于打滑检测
    ...
    if (abs_slip || rel_slip)
    {
        ...
        // TCS: 降低前馈补偿 (速度环本身有一定抗打滑能力)
        // 计算TCS因子: 偏差越大，前馈越小
        float slip_ratio = slip_ctrl.speed_deviation / (ref_speed + 100.0f);
        slip_ctrl.tcs_factor = fmaxf(1.0f - slip_ratio, TCS_MIN_FACTOR);
...
void ChassisTask()
{
    ...
    case CHASSIS_ROTATE: // 自旋,同时保持全向机动;当前wz维持定值,后续增加不规则的变速策略
        //static int time=0;
        //time++;
        chassis_cmd_recv.wz=440*rotate_speed_buff;
    ...
    // 先计算真实速度(用于速度外环和打滑检测)
    // 注意: 必须在ForceCalculate之前调用!
    EstimateSpeed();
    
    // 2. 计算各轮目标速度 (送入电机速度环)
    MecanumCalculate();
    
    // 3. 全局观测器计算前馈补偿 (送入电机电流前馈)
    ...
    // VOFA+调试 
    // 发送: 目标vx, 融合vx, 轮速vx, 前馈lf, TCS因子, 是否打滑
}
