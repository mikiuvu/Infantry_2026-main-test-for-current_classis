...
static void GlobalObserverCalculate(float dt)
{
    ...
    // 实际速度 (融合后的底盘速度 mm/s)
    float actual_vx = chassis_real_vx_feedback;
    float actual_vy = chassis_real_vy_feedback;
...
{   
    ...
    if (cap_energy > 75 && chassis_cmd_recv.dash_mode == DASH_ON) {
...
static float EstimateSpeed()
{
    ...
    // 角速度wz
    chassis_real_wz_feedback = (wheel_lf + wheel_rf + wheel_lb + wheel_rb)
                               / (4.0f * L * DEGREE_2_RAD);
    ...
    return dt;
#else
    chassis_real_vx_feedback = wheel_vx;
    chassis_real_vy_feedback = wheel_vy;
...
#if defined(CHASSIS_BOARD) || defined(CHASSIS_ONLY)
    chassis_feedback_data.chassis_wz_imu = Chassis_INS_data->Gyro[2];
#else
    chassis_feedback_data.chassis_wz_imu = 0.0f;
#endif
