...
#define REFEREE_SOF 0xA5 // 起始字节，协议固定为 0xA5
#define Robot_Red 0
#define Robot_Blue 1
#define Communicate_Data_LEN 5 // 自定义交互数据长度，更改自定义交互协议时只需要同步修改这里
...
typedef enum
{
	LEN_HEADER = 5, // 帧头长度
	LEN_CMDID = 2,	// 命令码长度
	LEN_TAIL = 2,	// 帧尾 CRC16 长度

	LEN_CRC8 = 4, // 帧头 CRC8 校验长度 = SOF + DataLength + Seq
} JudgeFrameLength_e;
...
typedef struct
{
	uint16_t data_cmd_id; // 单个 cmd_id 的上行频率上限为 10Hz，请合理安排带宽
	uint16_t sender_ID;
...
typedef struct
{
	uint8_t data[Communicate_Data_LEN]; // 数据段长度 n 需要小于 113
