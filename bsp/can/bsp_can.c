...
// CAN2邮箱清理诊断变量
volatile uint32_t can2_abort_count = 0;     // CAN2邮箱终止次数
volatile uint32_t can2_busoff_recovery = 0; // CAN2 Bus-Off恢复次数

// 手动清除busoff，恢复较快但是导致发送函数过于臃肿
// uint8_t CANTransmit(CANInstance *_instance,uint8_t timeout)
// {
//     // 仅对CAN2进行邮箱状态检查和Bus-Off恢复
//     if (_instance->can_handle == &hcan2)
//     {
//         uint32_t esr = hcan2.Instance->ESR;
        
//         // 检查是否Bus-Off (BOFF位，bit 2)
//         if (esr & 0x04)
//         {
//             // Bus-Off状态，需要清空所有邮箱并重启CAN
...
//             HAL_CAN_Stop(&hcan2);
//             HAL_CAN_Start(&hcan2);
//             can2_busoff_recovery++;
            
//             return 0; // 本次发送失败，下次会重试
//         }
        
//         // 检查是否有邮箱发送失败（TERR位）
//         uint32_t tsr = hcan2.Instance->TSR;
//         if (tsr & 0x08)  // TERR0 - 邮箱0发送错误
//         {
//             HAL_CAN_AbortTxRequest(&hcan2, CAN_TX_MAILBOX0);
//             can2_abort_count++;
//         }
//         if (tsr & 0x800) // TERR1 - 邮箱1发送错误
//         {
//             HAL_CAN_AbortTxRequest(&hcan2, CAN_TX_MAILBOX1);
//             can2_abort_count++;
//         }
//         if (tsr & 0x80000) // TERR2 - 邮箱2发送错误
//         {
//             HAL_CAN_AbortTxRequest(&hcan2, CAN_TX_MAILBOX2);
//             can2_abort_count++;
//         }
...
//     HAL_StatusTypeDef status = HAL_CAN_AddTxMessage(_instance->can_handle, &_instance->txconf, _instance->tx_buff, &_instance->tx_mailbox);
    
//     if (status != HAL_OK)
//     {
//         // HAL返回错误，如果是CAN2，清理对应邮箱
//         if (_instance->can_handle == &hcan2)
//         {
//             HAL_CAN_AbortTxRequest(&hcan2, CAN_TX_MAILBOX0);
//             HAL_CAN_AbortTxRequest(&hcan2, CAN_TX_MAILBOX1);
